...
class SyncleoUdpDevice:
    ...
    @classmethod
    def from_zeroconf(cls, ip: str, port: int, txt: dict[str, Any]) -> 'SyncleoUdpDevice':
        ...
        if protocol >= 2 and (curve != 29 or pubkey_length != 32):
            _LOGGER.warning(f"Device at {ip}:{port} advertises protocol version {protocol} with curve {curve} and public key size {pubkey_length}. Fallback to protocol 1.")
            protocol = 1
