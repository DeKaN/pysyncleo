...
class CryptoV2Encoder(SyncleoEncoder):
    ...
    def __init__(self, device):
        ...
        self.pubkey = bytes(reversed(self.privkey.public_key().public_bytes(
            encoding=srlz.Encoding.Raw, format=srlz.PublicFormat.Raw
        )))
        
        remote_pub = X25519PublicKey.from_public_bytes(bytes(reversed(device.device_pubkey)))
    ...
    def generate_handshake(self) -> bytes:
        ...
        encrypted_token = encryptor.update(self.device.device_token) + encryptor.finalize()
    ...
    def encode(self, seq: int, frame_type: FrameType, payload: bytes) -> bytes:
        if frame_type == FrameType.CMD and len(payload) > 0:
            ...
            ciphertext = encryptor.update(padder.update(bytes(newdata)) + padder.finalize()) + encryptor.finalize()
